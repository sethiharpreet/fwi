...
int main(int argc, char *argv[])
{
   ...
   omp_init();
	...
	sxbeg=230, szbeg=150, gxbeg=0, gzbeg=4, jsx=0, jsz=0, jgx=1, jgz=0 ;
	dx=0.005, dz=0.005,fm=10, dt=0.0005, amp=1.0, nb=60, hfop=2, ompchunk=1 ;
	niter=1,beta=0.0,epsil=0.0,alpha=0.0;
	// End
		...
		for(is=0; is<ns; is++){
			...
			for(it=0; it<nt; it++){
				...
				for(it=nt-1; it>-1; it--){
					...
					if(it==1000){ 
						printf("saving snapshot at t=%f s\n",it*dt);
						writebin(lap[0],fdm->nxpad*fdm->nzpad,"rsnap.bin");
					};
